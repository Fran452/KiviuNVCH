...
function ModalPlanes(props) {
  ...
  const handleSubmit = async (e) => {
    ...
    if (Object.keys(newErrors).length === 0){
      ...
      try {
        const res = await fetch(`${process.env.REACT_APP_API_URL}/apis/plan-accion/addTask`, {
          method: "POST",
          headers: {
  ...
  const handleChangeTask = async (e) => {
    ...
    if (Object.keys(newErrors).length === 0){
      ...
      try {
        const res = await fetch(`${process.env.REACT_APP_API_URL}/apis/plan-accion/modTask`, {
          method: "PUT",
          headers: {
              "Content-Type": "application/json"
          },
